...
func TestFilterExpressionParser(t *testing.T) {
	...
	assert.NotNil(fe.FilterExpr.SubFilterExpr)
	assert.Equal(2, len(fe.FilterExpr.SubFilterExpr.SubFilterExpr.FilterExpr.AndConditions))
	assert.NotNil(fe.FilterExpr.SubFilterExpr.AndContinuation)
	assert.Equal(1, len(fe.FilterExpr.SubFilterExpr.AndContinuation.FilterExpr.AndConditions))
	expr, err = fe.OutputExpression()
	assert.Nil(err)
