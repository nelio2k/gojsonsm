...
// FilterExpression         = ( "(" FilterExpression ")" { "AND" FilterExpression } { "OR" FilterExpression } ) | InnerExpression
// InnerExpression          = ( AndCondition { "OR" AndCondition } ) { "AND" FilterExpression }
...
func (f *FilterExpression) GetTotalOpenParens() (count int) {
	...
	if f.SubFilterExpr != nil {
		...
		count += f.FilterExpr.GetTotalOpenParens()
	}
	return
}
...
func (f *FilterExpression) GetTotalCloseParens() (count int) {
	...
	if f.SubFilterExpr != nil {
		...
		count += f.FilterExpr.GetTotalCloseParens()
	}
	return
}
...
func (f *FEInnerExpression) GetTotalOpenParens() (count int) {
	if f.SubFilterExpr != nil {
		count += f.SubFilterExpr.GetTotalOpenParens()
	}
	return
...
func (f *FEInnerExpression) GetTotalCloseParens() (count int) {
	if f.SubFilterExpr != nil {
		count += f.SubFilterExpr.GetTotalCloseParens()
	}
	return
...
func (fe *FEInnerExpression) String() string {
	...
	if fe.SubFilterExpr != nil {
		output = append(output, OperatorAnd)
		output = append(output, fe.SubFilterExpr.String())
	}
	return strings.Join(output, " ")
}

func (f *FEInnerExpression) outputExpressionNoParenCheck() (Expression, error) {
	var outExpr OrExpr
	...
	if f.SubFilterExpr != nil {
		...
		subExpr, err := f.SubFilterExpr.outputExpressionNoParenCheck()
		if err != nil {
			return nil, err
		}
